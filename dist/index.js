    ...
    try {
        if (options.json) {
            ...
            if (options.discovery) {
                discoveryResult = await fs.readJSON(options.discovery);
            }
            else {
                console.log(pc.yellow('  No discovery result provided.'));
                console.log(pc.dim('  Running discovery first...\n'));
...
async function runScaffoldJsonMode(options) {
    ...
    const discoveryResult = await fs.readJSON(options.discovery);
...
program
    ...
    try {
        ...
        await fs.writeFile(path.join(superskillsDir, '.env'), `# SuperSkills API key (saved by superskills init)\nANTHROPIC_API_KEY=${apiKey}\n`);
        process.env.ANTHROPIC_API_KEY = apiKey;
        p.log.success(`API key saved to ~/.superskills/.env`);
