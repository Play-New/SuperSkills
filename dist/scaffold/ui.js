...
export async function runScaffold(discovery, tools) {
    ...
    const fullPath = path.resolve(outputDir, projectName);
    console.log('');
    p.note(pc.bold('Project: ') + projectName + '\n' +
        ...
        pc.dim('  - Delivery integrations'), 'Scaffold Preview');
    ...
    try {
        const result = await scaffold({ discovery, tools }, { outputDir: outputDir });
...
function displayResults(result) {
    ...
    p.note(pc.cyan(`1. cd ${projectDir}`) + '\n' +
        ...
        pc.dim('   /strategy-init  /design-init  /trust-init  /testing-init'), 'Next Steps');
    // Env var guide
    const envGuide = [
    ...
    p.note(envGuide.map(([name, where]) => pc.bold(name) + '\n' + pc.dim(`  ${where}`)).join('\n\n'), 'Where to get each key');
    // Skills info
    p.note(pc.bold('Skills configured:\n') +
        result.agentTeam.skills.map(s => pc.dim(`  - ${s.name}: ${s.focus}`)).join('\n') + '\n\n' +
        pc.bold('Hooks:\n') +
        ...
        pc.dim('  - Stop: testing verification'), 'SuperSkills');
}
