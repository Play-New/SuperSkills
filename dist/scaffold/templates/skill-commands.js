...
1. **Ask for SHADCNBLOCKS_API_KEY** (optional, for pro blocks):
   - "Do you have a shadcnblocks API key? (Get one at shadcnblocks.com/dashboard)"
   - If yes: save to \`.env.local\` as SHADCNBLOCKS_API_KEY
   - If no: continue with free blocks only
...
export function generateSkillCommands(discovery) {
    ...
    for (const skill of [...INIT_SKILLS, ...REVIEW_SKILLS]) {
        files[`.claude/skills/${skill.name}/SKILL.md`] = skill.content(discovery);
    }
    return files;
}
