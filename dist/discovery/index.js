...
async function chooseInputMode() {
    const mode = await p.select({
        message: 'How would you like to provide project information?',
        options: [
            { value: 'paste', label: 'Paste text', hint: 'Brief, email, notes' },
            { value: 'file', label: 'Load file', hint: 'Text or Markdown' },
            { value: 'questions', label: 'Answer questions', hint: 'Guided flow' }
        ]
    });
    if (p.isCancel(mode)) {
        p.cancel('Cancelled');
...
async function handlePasteInput() {
    ...
    if (p.isCancel(content))
        return null;
    return runAnalysis(() => processRawContent(content, 'text'));
}
...
async function handleQuestionsInput() {
    ...
    const input = {
        ...
        currentProcess: parseList(processText),
        availableData: parseList(dataText)
    };
...
async function collectContext() {
    const forWhom = await p.select({
        ...
        options: [
            { value: 'me', label: 'Me', hint: 'Personal or freelance' },
            { value: 'my_company', label: 'My company', hint: 'Your own business' },
            { value: 'client', label: 'A client', hint: 'External company' }
        ]
    });
    if (p.isCancel(forWhom))
        return null;
    let companyName;
    if (forWhom !== 'me') {
    ...
    if (p.isCancel(businessDescription))
        return null;
...
async function runAnalysis(analyzeFn) {
...
function displayResult(result) {
    console.log('');
    p.note(pc.bold('Industry Context\n') +
    ...
    p.note(pc.bold('Enrichment\n') +
        ...
        pc.dim(`Triggers: ${result.eiidMapping.delivery.triggers.join(', ')}`), 'AI-Native Architecture (EIID)');
}
